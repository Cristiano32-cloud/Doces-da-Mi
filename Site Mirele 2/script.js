...
function addProductToCart(event) {
    ...
    const productTitle = productInfos.getElementsByClassName("product")[0].innerText
    ...
    for (var i = 0; i < productsCartName.length; i++) {
        if (productsCartName[i].innerText === productTitle) {
            productsCartName[i].parentElement.getElementsByClassName("product-input")[0].value++
            return  
        }
    }
    ...
    updateTotal()
    newCartProduct.getElementsByClassName("product-input")[0].addEventListener("change", checkIfInputIsNull)
...
for (var i = 0; i < cartProducts.length; i++) {
    ...
    const productQuantity = cartProducts[i].getElementsByClassName("product-input")[0].value
